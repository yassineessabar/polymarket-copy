...
export default function WalletPage() {
  ...
  return (
    <div className="max-w-[600px] mx-auto">
      ...
      <div className="bg-white rounded-2xl p-4 sm:p-5 mb-4 border border-black/[0.04]">
        <p className="text-xs text-[#6B7280] uppercase tracking-wider font-medium mb-3">Trading Mode</p>
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={switchToDemo}
            disabled={saving}
            className={`p-4 rounded-xl border-2 text-left transition-all ${
              isDemo ? "border-[#0F0F0F] bg-[#F5F5F5]" : "border-transparent bg-[#F5F5F5] hover:border-[#E0E0E0]"
            }`}
          >
            <p className="text-sm font-bold text-[#0F0F0F]">Demo</p>
            <p className="text-xs text-[#6B7280] mt-0.5">Virtual funds, zero risk</p>
          ...
          >
            <p className="text-sm font-bold text-[#0F0F0F]">Live</p>
            <p className="text-xs text-[#6B7280] mt-0.5">Real USDC, real profits</p>
            {!isDemo && <p className="text-xs font-bold text-[#009D55] mt-2">Active</p>}
          </button>
        </div>
      </div>
      ...
      {isDemo && (
        <>
          {/* Demo Balance Card */}
          <div className="bg-white rounded-2xl p-6 sm:p-8 mb-4 text-center border border-black/[0.04]">
            <div className="text-xs text-[#6B7280] uppercase tracking-wider mb-2 font-medium">
              Demo Balance
            </div>
            <div className="text-3xl sm:text-4xl font-bold font-mono tracking-tight text-[#0F0F0F]">
            ...
            <div className="mt-4 pt-4 border-t border-black/5 flex items-center justify-center gap-3">
              <input
                ...
                className="bg-[#F5F5F5] border border-black/5 rounded-xl px-5 py-2.5 text-[#0F0F0F] outline-none focus:border-[#0F0F0F] w-full max-w-[180px] text-sm text-center font-mono"
              />
              <button
                onClick={() => saveTradingMode({ demo_balance: settings.demo_balance || 1000 })}
                className="text-xs font-medium text-[#0F0F0F] bg-[#F5F5F5] border border-black/5 rounded-xl px-4 py-2.5 hover:bg-[#EBEBEB] transition-colors"
              >
                Update
              </button>
            </div>
          ...
          {/* Demo info */}
          <div className="bg-white rounded-2xl p-5 mb-4 border border-black/[0.04]">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-[#0F0F0F] mb-1">You&apos;re in demo mode</p>
            ...
            <button
              ...
              className="mt-4 w-full border border-[#DC2626]/30 text-[#DC2626] font-medium py-2.5 rounded-xl transition-all text-sm hover:bg-[#DC2626]/5"
            >
              Reset Demo Account
            </button>
          </div>
      ...
      {!isDemo && (
        <>
          ...
          <div className="bg-white rounded-2xl p-6 sm:p-8 mb-4 text-center border border-black/[0.04]">
            ...
            <div className="text-xs text-[#6B7280] mt-1 font-medium">on Polygon Network</div>
          </div>

          {/* Deposit button */}
          <button
            onClick={() => { setShowDeposit(true); }}
            className="w-full bg-[#009D55] hover:bg-[#008548] text-white rounded-2xl p-5 text-left transition-all border border-black/[0.04] mb-4 flex items-center gap-4"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-[500px] rounded-t-3xl sm:rounded-3xl max-h-[85vh] overflow-y-auto p-5 sm:p-6 shadow-xl">
            ...
            <button
              ...
              className="absolute top-4 right-4 w-8 h-8 rounded-xl bg-[#F5F5F5] hover:bg-[#EBEBEB] flex items-center justify-center transition-colors"
            >
            ...
            <h2 className="text-lg font-bold text-[#0F0F0F] mb-5">Fund Your Wallet</h2>
            ...
            <div className="flex bg-[#F5F5F5] rounded-xl p-1 mb-5">
              {(["card", "crypto"] as const).map((tab) => (
                <button
                  ...
                  className={`flex-1 text-sm font-medium py-2 rounded-xl transition-all capitalize ${
                    depositTab === tab
                      ? "bg-[#0F0F0F] text-white border border-black/[0.04]"
                ...
                <div className="space-y-2">
                  {CHAINS.map((chain) => (
                    <button
                      ...
                      className="w-full flex items-center gap-4 p-4 bg-[#F5F5F5] hover:bg-[#EBEBEB] rounded-2xl transition-all text-left"
                    >
                      <div className="flex-shrink-0">{chain.icon}</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <p className="text-sm font-bold text-[#0F0F0F]">{chain.name}</p>
                          <p className="text-xs text-[#6B7280]">Min: {chain.minDeposit}</p>
                        </div>
                        <p className="text-xs text-[#6B7280] mt-0.5">{chain.description}</p>
                      </div>
            ...
            {depositTab === "crypto" && selectedChain && (
              <div>
                <button
                  ...
                  className="flex items-center gap-1 text-sm text-[#6B7280] hover:text-[#0F0F0F] mb-4 transition-colors font-medium"
                >
                ...
                <div className="text-center mb-4">
                  <div className="inline-flex items-center gap-2 bg-[#F5F5F5] rounded-xl px-4 py-2 mb-4">
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="w-full">
                    <label className="text-xs text-[#6B7280] mb-1.5 block font-medium">
                      Your deposit address
                    </label>
                    <div className="bg-[#F5F5F5] border border-black/5 rounded-2xl px-4 py-3 font-mono text-[11px] sm:text-xs break-all text-[#0F0F0F] mb-3">
                      {walletAddress}
                    </div>
                    <button
                      onClick={copyAddress}
                      className="w-full bg-[#0F0F0F] hover:bg-[#333] text-white font-medium py-3 rounded-xl transition-all text-sm flex items-center justify-center gap-2"
                    >
                      {copied ? (
                        <>
      ...
      {!isDemo && <div className="bg-white rounded-2xl p-5 sm:p-8 mb-4 border border-black/[0.04]">
        <h3 className="font-bold text-sm sm:text-base mb-4 text-[#0F0F0F]">Your Wallet</h3>
        ...
        <div className="flex flex-col items-center gap-5">
          ...
          <div className="w-full">
            <div className="bg-[#F5F5F5] rounded-2xl px-4 py-3 font-mono text-[11px] sm:text-sm break-all text-center mb-4 text-[#0F0F0F]">
              {walletAddress}
            </div>
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm px-5">
          <div className="bg-white rounded-2xl p-6 w-full max-w-[380px] shadow-lg">
            <h3 className="font-bold text-base text-[#0F0F0F] mb-2">Reset Demo Account?</h3>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 border border-[#0F0F0F] text-[#0F0F0F] font-medium py-2.5 rounded-xl text-sm hover:bg-[#F5F5F5]"
              >
                Cancel
              </button>
              <button
                onClick={resetDemo}
                disabled={resetting}
                className="flex-1 bg-[#DC2626] hover:bg-[#B91C1C] text-white font-medium py-2.5 rounded-xl text-sm disabled:opacity-50"
              >
      ...
      <div className="text-center pb-6">
        <button
          ...
          disabled={saving}
          className="text-xs text-[#6B7280] hover:text-[#0F0F0F] font-medium transition-colors disabled:opacity-50"
        >
